...
export function ToolConvertLinks() {
  ...
  const { convertLinks, useLinks } = useMemo(() => {
    if (!pathname) return { convertLinks: [], useLinks: [] };
    ...
    return { convertLinks, useLinks };
  }, [pathname]);

  if (convertLinks.length === 0 && useLinks.length === 0) return null;
