...
export async function generateMetadata({ params }: PageProps): Promise<Metadata> {
  ...
  if (!conv) return { title: 'Conversion not found · Web Toolkit' };
  ...
  return {
    ...
    alternates: {
      canonical: enUrl,
      languages: { 'ko-KR': koUrl, en: enUrl, 'x-default': enUrl },
    },
    openGraph: {
      title: content.title,
      description: content.description,
...
export default async function ConvertPageEn({ params }: PageProps) {
  const { slug } = await params;
  const conv = getConversion(slug);
  if (!conv) notFound();
  const content = buildConversionContent(conv, 'en');
