...
/** CRC32 룩업 테이블(표준 다항식 0xEDB88320, 반사형). 모듈 로드 시 1회 생성. */
...
/** UTF-8 바이트열의 CRC32 체크섬(부호 없는 32비트). */
...
export default function Crc32HashPage() {
  const [input, setInput] = useState('');
  const [copied, setCopied] = useState<CopyField | null>(null);

  const result = useMemo<Crc32Result | null>(() => {
  ...
  async function copyField(field: CopyField): Promise<void> {
    ...
    try {
      ...
      console.error('[crc32-hash] clipboard write failed', err);
    }
  }
    ...
    <div className="min-h-dvh bg-background">
      ...
      <main className="mx-auto max-w-3xl space-y-4 p-4">
        <p className="text-sm text-muted-foreground">
          입력한 텍스트의 CRC32 체크섬(표준 다항식 0xEDB88320)을 16진수와 10진수로 계산합니다.
        </p>
        ...
        <div className="rounded-lg border border-amber-500/40 bg-amber-500/10 p-3 text-xs leading-relaxed text-muted-foreground">
          ⚠️ CRC32 는 전송·저장 오류를 잡는 무결성 체크섬입니다. 위변조 방지·보안 용도에는
          SHA-256 이상을 사용하세요.
        </div>
