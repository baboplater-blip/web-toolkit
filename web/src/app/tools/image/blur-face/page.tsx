...
export default function BlurFacePage() {
  ...
  const detectFaces = useCallback(async (info: LoadedImage) => {
    ...
    try {
      ...
      setDetectStatus('WASM 초기화 중');
      const vision = await FilesetResolver.forVisionTasks(WASM_CDN);
      setDetectStatus('감지 모델 로드 중 (~500KB)');
      ...
      (result.detections ?? []).forEach((d, i) => {
        ...
        detected.push({
          ...
          w: Math.min(info.width, Math.round(bb.width + padX * 2)),
          h: Math.min(info.height, Math.round(bb.height + padY * 2)),
          source: 'auto',
          enabled: true,
    ...
    try {
      ...
      for (const box of enabled) {
        if (blurShape === 'pixelate') {
          ...
          tctx.drawImage(loaded.element, box.x, box.y, box.w, box.h, 0, 0, smallW, smallH);
          ctx.save();
          if (blurShape === 'pixelate' /* always rect clip for now */) {
            ctx.beginPath();
          ...
          // gaussian blur: clip 영역에 필터 적용하여 원본 재drawing
          ctx.save();
          ctx.beginPath();
