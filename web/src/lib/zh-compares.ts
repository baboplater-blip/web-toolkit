...
export const COMPARES_ZH: Compare[] = [
  ...
  },
  {
    slug: 'nanoid-vs-uuid',
    category: 'dev',
    title: 'Nano ID vs UUID — 你该用哪种 ID 格式?',
    h1: 'Nano ID vs UUID',
    description:
      'Nano ID 生成短小、URL 安全的标识符;UUID 生成标准的 128 位标识符。比较长度、可读性与碰撞概率 — 免费,在浏览器中完成。',
    intro:
      '两者都用强随机性生成唯一标识符,但在长度与通用性上各有取舍。Nano ID 默认短小且 URL 安全 — 通常是从 64 个字符的字母表中取出的 21 个字符 — 因此很适合放进链接和 slug。UUID 则是历史悠久的 128 位标准(RFC 4122),通常写成带连字符的 36 个字符,几乎所有数据库和语言都认得它。两者都不会上传任何内容,全部在浏览器本地完成。',
    options: [
      {
        label: 'Nano ID',
        toolId: 'nanoid-gen',
        best: '用于 URL、slug 和公开引用的短小、链接友好的 id。',
        pros: [
          '短小 — 约 21 个字符,而 UUID 是 36 个',
          'URL 安全字母表(A–Z、a–z、0–9、_ 和 -),无需额外编码',
          '可调长度与字母表,自行权衡碰撞概率与长度',
          '使用安全随机性生成',
        ],
        cons: [
          '并非公认标准,部分系统会期望 UUID 的形态',
          '没有内置的版本或时间戳语义',
        ],
      },
      {
        label: 'UUID',
        toolId: 'uuid-gen',
        best: '需要公认的 128 位标准时的数据库主键和 API。',
        pros: [
          'RFC 4122 标准,几乎所有数据库和语言都能理解',
          'PostgreSQL 等数据库有原生的 UUID 列类型',
          '带版本(v4 随机、v1 基于时间),语义广为人知',
          'v4 的碰撞概率极低',
        ],
        cons: [
          '带连字符的 36 个字符 — 偏长,放进 URL 不够美观',
          '透露的信息很少,却在索引和链接里占更多空间',
        ],
      },
    ],
    verdict:
      'id 会出现在 URL、slug 或任何在意长度与可读性的地方、且两端都由你掌控时,选 Nano ID;需要公认标准时 — 数据库主键、公开 API 约定,或要和期望 UUID 格式的工具互通 — 选 UUID。常见做法是内部用 UUID,对外的短链用 Nano ID。',
    faqs: [
      {
        q: 'Nano ID 的抗碰撞能力和 UUID 一样吗?',
        a: '在默认的 21 个字符下,Nano ID 的随机量级与 UUID v4 相近。两者都基于安全随机性;若把 Nano ID 缩短,就是用一部分碰撞安全换取简短,所以高频场景请保持默认长度。',
      },
      {
        q: '能把 Nano ID 存进期望 UUID 的列里吗?',
        a: '不能。UUID 列期望固定的 128 位 UUID 格式。请把 Nano ID 存进普通的 text/varchar 列,或者在需要原生类型时改用 UUID。',
      },
    ],
    keywords: ['nanoid vs uuid', '短 唯一 id', 'url 安全 id', 'uuid 替代'],
  },
  {
    slug: 'base58-vs-base64',
    category: 'dev',
    title: 'Base58 vs Base64 — 你该用哪种编码?',
    h1: 'Base58 vs Base64',
    description:
      'Base58 去掉了易混淆字符,生成人类友好的 id(比特币在用);Base64 是紧凑、通用的二进制转文本标准。两者对比,在浏览器中完成。',
    intro:
      '两者都把原始字节转成可打印文本,但优化目标不同。Base58 使用 58 个字符的字母表,刻意去掉了易混淆的 0、O、I、l 以及 + 和 /,因此字符串更便于念出、手抄或放进 URL — 这也是比特币地址和许多加密标识符采用它的原因。Base64 使用 64 个字符的字母表,数据更紧凑(约 33% 开销),并在邮件、data URI 和 JSON 中处处可用。两者的编码与解码都完全在浏览器本地进行。',
    options: [
      {
        label: 'Base58',
        toolId: 'base58',
        best: '供人复制、输入或念出的可读 id — 地址、短密钥。',
        pros: [
          '没有易混淆字符(去掉了 0/O、I/l/1)— 抄写出错更少',
          '没有 + 或 /,输出对 URL 和文件名安全,无需转义',
          '比特币地址和许多加密标识符背后用的就是它',
        ],
        cons: [
          '同样数据下输出比 Base64 略大',
          '各语言和库的内置支持不如 Base64 普及',
          '不适合大块二进制数据',
        ],
      },
      {
        label: 'Base64',
        toolId: 'base64',
        best: '嵌入或传输任意二进制数据 — data URI、邮件、JSON。',
        pros: [
          '紧凑:约 33% 开销,比 Base58 更密',
          '几乎每种语言和平台都内置',
          'data URI、MIME 邮件和在文本中嵌入二进制的标准做法',
        ],
        cons: [
          '含有 0/O、I/l 以及 + /,易抄错或需要 URL 转义',
          '标准字母表不是 URL 安全的,除非用 URL 安全变体',
        ],
      },
    ],
    verdict:
      '当字符串要由人来读、输入或分享时 — 钱包地址、短 id 以及任何要念出来的内容 — 想避开易混淆字符就选 Base58;当你要把任意二进制数据经文本通道传输时(data URI、邮件附件、JSON 字段),想要最紧凑、最通用的编码就选 Base64。',
    faqs: [
      {
        q: '比特币为什么用 Base58 而不是 Base64?',
        a: 'Base58 去掉了易混淆字符(0/O、I/l)以及 + 和 / 这类在 URL 中或复制时会出问题的符号,使手抄的地址出错概率大大降低 — 当一个笔误就可能把资金转错地方时,这一点很重要。',
      },
      {
        q: 'Base58 和 Base64 哪个更短?',
        a: 'Base64 输出更短,因为它的字母表更大,每个字符承载更多比特。Base58 用一点长度换取可读性和复制安全。',
      },
    ],
    keywords: ['base58 vs base64', 'base58 编码', '比特币 base58', '二进制 转 文本 编码'],
  },
  {
    slug: 'text-shadow-vs-box-shadow',
    category: 'dev',
    title: 'text-shadow vs box-shadow — 你需要哪种 CSS 阴影?',
    h1: 'text-shadow vs box-shadow',
    description:
      'text-shadow 给文字的字形投影;box-shadow 给元素的矩形盒子投影。可视化生成任意一种 — 免费,在浏览器中完成。',
    intro:
      '这两个 CSS 属性都能制造阴影,但作用对象不同。text-shadow 画出贴合字符实际形状的阴影,因此用于标题、标签以及任何想从页面上凸显出来的文字。box-shadow 画的是元素盒子(它的边框轮廓)周围的阴影,因此用于卡片、按钮、弹窗和面板。两个生成器都可调节偏移、模糊和颜色,并复制可直接粘贴的 CSS。',
    options: [
      {
        label: 'text-shadow',
        toolId: 'text-shadow',
        best: '给文字字形增加层次、光晕或描边。',
        pros: [
          '阴影贴合字形,而不是矩形',
          '适合标题、霓虹光晕,以及图片之上的可读文字',
          '支持以逗号分隔的多重阴影,做描边或叠加效果',
        ],
        cons: [
          '没有扩展半径(spread,不同于 box-shadow)',
          '只影响文字,不影响元素的盒子',
        ],
      },
      {
        label: 'box-shadow',
        toolId: 'box-shadow',
        best: '让卡片、按钮和面板从页面上抬起。',
        pros: [
          '给元素盒子投影,并跟随其 border-radius',
          '可加扩展半径和 inset 选项做内阴影',
          '在 UI 中营造层次与高度的标准做法',
        ],
        cons: [
          '跟随矩形盒子,无法描出文字形状',
    ...
    verdict:
      '阴影应贴合字母时 — 标题、说明文字、光晕效果,或叠在杂乱照片上的文字 — 用 text-shadow;要抬起整个元素时 — 卡片、按钮、下拉或弹窗 — 用 box-shadow。两者不可互换,也可以一起用(有阴影的卡片里放有阴影的文字)。',
    faqs: [
      {
        q: 'box-shadow 能给文字加阴影吗?',
        a: '加不到字形本身 — box-shadow 跟随元素的矩形盒子。要给字母形状投影,请用 text-shadow。',
      },
      {
        q: 'text-shadow 像 box-shadow 那样支持扩展半径吗?',
        a: '不支持。text-shadow 只接受 offset-x、offset-y、blur 和颜色。需要扩展半径或内阴影时,那是 box-shadow 的功能。',
      },
    ],
    keywords: ['text-shadow vs box-shadow', 'css 文字 阴影', 'css 盒子 阴影', 'css 阴影 生成器'],
  },
  {
    slug: 'card-type-vs-card-validate',
    category: 'security',
    title: '卡类型识别 vs 卡号校验 — 识别还是校验?',
    h1: '信用卡类型 vs 卡号校验',
    description:
      '从开头数字识别卡品牌,或用 Luhn 校验和检查卡号。看看各自做什么 — 免费,在浏览器中完成,不上传任何内容。',
    intro:
      '这两个工具回答的是关于卡号的不同问题,且都不会联系银行。类型识别读取开头数字(IIN/BIN 前缀)来推测品牌 — Visa、Mastercard、Amex 等 — 便于在输入时显示对应的卡片标志。校验工具运行 Luhn 算法,检查这些数字是否构成结构上有效的卡号(发现笔误或数字颠倒)。两者都只是格式检查;它们不确认卡是否真实存在、是否有效,也不确认是否有余额。',
    options: [
      {
        label: '信用卡类型',
      ...
      },
      {
        label: '卡号校验(Luhn)',
        toolId: 'cc-validate',
        best: '提交前用 Luhn 校验和发现笔误。',
        pros: [
          '运行每张真实卡号都使用的 Luhn 校验',
          '即时发现输错或颠倒的数字',
          '与品牌识别搭配,使表单校验更友好',
        ],
        cons: [
    ...
    verdict:
      '用类型识别来显示卡号属于哪个品牌(用于标志和品牌专属规则),用 Luhn 校验在提交前发现明显的笔误。在真实表单里通常两者并用:先识别品牌以格式化和标注字段,再校验校验和。两个工具都不会授权支付 — 只有支付处理方才能确认卡是否真实且有余额。',
    faqs: [
      {
        q: 'Luhn 校验和通过就代表卡是真的吗?',
        a: '不是。Luhn 只确认这些数字内部一致,可发现笔误;它无法判断该卡是否曾被发行、是否有效或是否有余额 — 只有发卡银行或你的支付处理方才能确认。',
      },
      {
        q: '应该两个检查一起做吗?',
        a: '应该。先从前缀识别品牌以显示正确标志并套用品牌规则,再运行 Luhn 校验,在提交前拒绝输错的卡号。',
      },
    ],
    keywords: ['卡类型 vs 卡号校验', '识别 卡 品牌', 'luhn 校验 卡号', '信用卡 bin'],
  },
  {
    slug: 'ean-vs-isbn',
    category: 'security',
    title: 'EAN vs ISBN — 你在校验哪种代码?',
    h1: 'EAN vs ISBN',
    description:
      'EAN 校验零售商品条码(GTIN/EAN-13);ISBN 校验图书标识符。两者都检查校验位 — 免费,在浏览器中完成。',
    intro:
      'EAN 和 ISBN 是密切相关的标识方案,都以一位校验位结尾。EAN(属于 GTIN 家族)是零售商品上的条码 — 食品、电子产品、包装 — 通常 13 位。ISBN 标识某一图书的特定版本。两者天然重叠:现代的 ISBN-13 实际上就是 978/979(Bookland)前缀段内的 EAN-13,且 ISBN-13 和 EAN-13 都用 mod-10 校验和。较旧的 ISBN-10 则不同 — 它使用 mod-11 校验和,末位字符可以是 X。这些工具只校验校验位和格式,不判断该商品或图书是否真实存在。',
    options: [
      {
        label: 'EAN / GTIN',
        toolId: 'ean-validate',
        best: '检查零售商品条码(EAN-13、EAN-8、UPC/GTIN)。',
        pros: [
          '校验 GTIN/EAN 的 mod-10 校验位',
          '覆盖包装商品上日常的零售条码',
          '发现输错或扫错的数字',
        ],
        cons: [
          '确认的是校验和,而非商品已注册或真实存在',
          '不针对图书 — 图书元数据请用 ISBN 规则',
        ],
      },
      {
        label: 'ISBN',
        toolId: 'isbn-validate',
        best: '检查图书标识符 — ISBN-13 和旧版 ISBN-10。',
        pros: [
          '处理 ISBN-13(mod-10)和 ISBN-10(mod-11,X 校验位)',
          'ISBN-13 在 978/979 段内即为 EAN-13,因而与零售相通',
          '发现目录与图书馆数据中的抄写错误',
        ],
        cons: [
          '校验位有效不证明该书存在或在印',
          'ISBN-10 用的是与零售条码不同的 mod-11 校验',
        ],
      },
    ],
    verdict:
      '在校验实体零售商品上的条码?用 EAN/GTIN 检查。在校验图书标识符 — 尤其是 10 位代码,或想把它当作图书来处理?用 ISBN 检查,它也能理解 mod-11 的 ISBN-10 形式。记住两者的重叠:以 978 或 979 开头的 ISBN-13 同时也是有效的 EAN-13。两个工具都只校验结构和校验位 — 不会去任何数据库里查找该条目。',
    faqs: [
      {
        q: 'ISBN-13 也是有效的 EAN 吗?',
        a: '是的。ISBN-13 是 978 或 979(Bookland)前缀段内的 EAN-13,两者共用同一个 mod-10 校验位。所以 978/979 的 ISBN-13 能通过 EAN-13 的校验和检查。',
      },
      {
        q: 'ISBN-10 为什么用不同的校验和?',
        a: 'ISBN-10 早于与 EAN-13 的对齐,使用 mod-11 加权校验和,末位字符可以是 X(代表 10)。EAN-13 和 ISBN-13 都使用更简单的 mod-10 方案。',
      },
    ],
    keywords: ['ean vs isbn', '校验 条码', 'isbn 校验和', 'gtin ean13'],
  },
  {
    slug: 'posterize-vs-threshold',
    category: 'image',
    title: '色调分离 vs 阈值二值化 — 你想要哪种图像效果?',
    h1: '色调分离 vs 阈值二值化',
    description:
      '色调分离把图像减为少数几级纯色;阈值二值化按亮度把图像转为纯黑白。两者对比 — 免费,在浏览器中完成。',
    intro:
      '两种效果都通过削减色调范围来简化图像,但程度大不相同。色调分离把每个通道量化成所选的若干级,于是平滑的渐变变成一块块纯色 — 一种保留色彩、像海报或丝网印刷的效果。阈值二值化则走到底:它测量每个像素的亮度,把它变成纯黑或纯白,产生强烈的双色、高对比结果。两者都完全在浏览器本地处理图像,不上传任何内容。',
    options: [
      {
        label: '色调分离',
        toolId: 'image-posterize',
        best: '保留色彩、一块块色带的海报风格。',
        pros: [
          '把每个通道减为你选择的 N 级',
          '保留色彩,同时把渐变压成色带',
          '适合丝网印刷、波普艺术和风格化图形',
    ...
    verdict:
      '想要风格化、彩色、一块块色带的海报感?用色调分离,并选保留几级。想要硬朗的黑白图 — 模板、剪影或徽标清理?用阈值二值化,并调节亮度临界点。色调分离保留色彩和多个级别;阈值只保留两级。你甚至可以先做色调分离得到图形感,再对副本做阈值得到匹配的蒙版。',
    faqs: [
      {
        q: '色调分离和阈值二值化有什么区别?',
        a: '色调分离把每个颜色通道减为少数几级纯色但保留色彩,产生色带状的海报效果。阈值二值化按每个像素的亮度,把整幅图减为两级 — 纯黑和纯白。',
      },
      {
        q: '做模板该用哪个?',
        a: '阈值二值化。它的纯黑白输出能给出干净的形状供裁剪或描摹。色调分离保留色彩和多个色调,更适合做风格化图形,而非模板。',
      },
    ],
    keywords: ['色调分离 vs 阈值', '图像 色调分离', '图像 阈值', '黑白 图像 效果'],
  },
  {
    slug: 'random-words-vs-lorem',
    category: 'text',
    title: '随机单词 vs Lorem Ipsum — 该用哪种占位文本?',
    h1: '随机单词 vs Lorem Ipsum',
    description:
      '随机单词输出真实、可读的英文单词;Lorem Ipsum 生成经典的中性拉丁文填充。挑对占位文本 — 免费,在浏览器中完成。',
    intro:
      '两者都生成填充文本,但词本身不同。随机单词生成器输出真实的英文单词,因此结果可读,适合需要可辨认语言的场景 — 单词表、提示、测试数据或快速示例。Lorem Ipsum 生成大家熟悉的伪拉丁文填充,读起来像中性的乱码,而这在设计中正是用意:它填满空间,又不会让评审被文字分心或误以为是真实内容。两者都在浏览器本地运行,不上传。',
    options: [
      {
        label: '随机单词',
        toolId: 'random-words',
        best: '用于列表、提示和测试数据的真实、可读英文单词。',
        pros: [
          '输出人们能认得的真实英文单词',
          '适合单词游戏、提示、口令短语灵感和样本数据',
          '可选择需要多少个单词',
        ],
        cons: [
          '真实单词可能让评审从版式上分心',
    ...
    verdict:
      '需要真正可读的英文时 — 示例列表、单词游戏、提示,或词本身有意义的测试数据 — 选随机单词;在做版式稿、想要中性填充以占住空间又不抢注意力时,选 Lorem Ipsum。设计师常用 Lorem;需要真实词汇的人则用随机单词。',
    faqs: [
      {
        q: '为什么用 Lorem Ipsum 而不是真实单词?',
        a: 'Lorem Ipsum 刻意没有含义,好让评审专注于版式和排印,而不是去读文案。真实单词往往把注意力引向内容本身,所以设计师更偏好中性填充。',
      },
      {
        q: '做测试数据哪个更好?',
        a: '想要可辨认的英文 token 来肉眼检查时用随机单词;只需要长度逼真的整块文本来填满设计时,Lorem Ipsum 更合适。',
      },
    ],
    keywords: ['随机单词 vs lorem ipsum', '占位 文本', 'lorem ipsum 替代', '随机 单词 生成器'],
  },
  {
    slug: 'business-days-vs-date-diff',
    category: 'util',
    title: '工作日 vs 日期差 — 算工作日还是日历天?',
    h1: '工作日 vs 日期差',
    description:
      '工作日计数只算工作日并跳过周末;日期差计算两个日期之间的整段日历天数。挑对那个 — 免费,在浏览器中完成。',
    intro:
      '两者都衡量两个日期之间的间隔,但计数方式不同。工作日计算器只算工作日、跳过周末,适用于交付预估、SLA 和项目排期。日期差计算器计算完整的日历跨度 — 每一天,或按年、月、日拆分 — 适用于年龄、纪念日和总时长。选错往往是差几天误差的常见来源。',
    options: [
      {
        label: '工作日',
        toolId: 'business-days',
        best: '跳过周末的工作日计数 — SLA、交付、截止日。',
        pros: [
          '只算周一至周五,跳过周末',
          '与交付窗口和 SLA 的报价方式一致',
          '更适合项目和周转时间预估',
    ...
    verdict:
      '在报周转时间、交付日期或 SLA?用工作日,别让周末把数字撑大。在量年龄、纪念日,或两个日期之间的总时长?用日期差算完整的日历跨度。只要范围里包含周末,两者的答案就会不同,所以要根据周末是否该计入来选对工具。',
    faqs: [
      {
        q: '这些工具会考虑法定假日吗?',
        a: '工作日计算器会跳过周末;法定假日因国家和地区而异,所以请确认是否需要手动减去特定假日。日期差计算器则不论假日,计算每一个日历天。',
      },
      {
        q: '为什么两者给出的数字不同?',
        a: '日期差计算所有日历天,包括周末;工作日则跳过周六和周日。范围内只要有周末,工作日的计数就会更小。',
      },
    ],
