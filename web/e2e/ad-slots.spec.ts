...
test.describe('광고 배너 슬롯', () => {
  test('상단 + 좌측 + 우측 슬롯이 표시 (xl 화면)', async ({ page }) => {
    ...
    const left = page.locator('[data-ad-slot="sidebarLeft"]');
    const right = page.locator('[data-ad-slot="sidebarRight"]');
  ...
  test('모바일에서는 사이드는 숨고 상단만 표시', async ({ page }) => {
    ...
    await page.goto('/tools');
    await expect(page.locator('[data-ad-slot="top"]')).toBeVisible();
    await expect(page.locator('[data-ad-slot="sidebarLeft"]')).toBeHidden();
    await expect(page.locator('[data-ad-slot="sidebarRight"]')).toBeHidden();
  });
