...
> 새 규칙 추가는 가능하나, 기존 규칙 완화·삭제는 **사용자 명시 승인** 필요. · 길잡이 [README.md](README.md)

---

## 🚦 치트시트 (한눈에 — 자세한 건 아래 A~H)

| 하면 안 되는 것 | 대신 |
|---|---|
| 사용자 파일 서버 전송 / API Route / Node 모듈 | 전부 브라우저 로컬(Web Worker+WASM) |
| `next/link` (정적 export) | 네이티브 `<a>` 태그 |
| 초기 렌더에 `Date.now`·`Math.random`·`localStorage` | 마운트 후 `useEffect` 또는 클릭 시 주입 |
| 다국어 키셋 어긋남 / 죽은 가이드 링크 | EN=JA=ZH 일치 · registry id 대조 |
| 게이트 빨간불에 커밋 | [test.md](test.md) 전부 초록 후 커밋 |
| 영어 보고서 / 비밀정보 평문 | 사용자 문서는 한국어 · 비밀은 절대 평문 금지 |

> "커밋 배포/배포해/푸시해" = origin/master 직접 push 승인.
